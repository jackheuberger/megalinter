...
ARG NPM_PACKAGE_JSON_LINT_VERSION=10.2.2
...
ARG KUBERNETES_KUBESCAPE_VERSION=4.0.5
...
RUN uv pip install --system --no-cache pip==${PIP_PIP_VERSION} virtualenv==${PIP_VIRTUALENV_VERSION} \
    ...
    && uv venv --seed --no-project --no-managed-python --no-cache "/venvs/nbqa" && VIRTUAL_ENV="/venvs/nbqa" uv pip install --no-cache nbqa==${PIP_NBQA_VERSION} mypy==${PIP_MYPY_VERSION} \
